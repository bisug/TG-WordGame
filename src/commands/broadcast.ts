...
// Called once at boot. An interrupted run leaves its state in Redis, and the
// audience is only half-messaged at that point, so the same run is re-driven
// from the processed set instead of being abandoned.

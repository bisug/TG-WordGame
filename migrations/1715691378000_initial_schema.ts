...
export async function up(db: Kysely<any>): Promise<void> {
  ...
  await db.schema
    ...
    .addColumn("should_recreate_on_expire", "boolean", (col) => col.defaultTo(false).notNull())
    .addColumn("allowed_lengths", sql`integer[]`, (col) => col.defaultTo(sql`ARRAY[5,4,6]`).notNull())
    .addColumn("created_at", "timestamptz", (col) => col.defaultTo(sql`now()`).notNull())
    .addColumn("updated_at", "timestamptz", (col) => col.defaultTo(sql`now()`).notNull())
  ...
  await db.schema
    ...
    .addColumn("chat_id", "text", (col) => col.notNull().references("broadcast_chats.id").onDelete("cascade"))
  ...
  await db.schema
    ...
    .addColumn("user_id", "text", (col) => col.notNull().primaryKey().references("users.id").onDelete("cascade"))
    .addColumn("created_at", "timestamptz", (col) => col.defaultTo(sql`now()`).notNull())
    .addColumn("updated_at", "timestamptz", (col) => col.defaultTo(sql`now()`).notNull())
    .execute();
  ...
  await db.schema
    ...
    .addColumn("created_at", "timestamptz", (col) => col.defaultTo(sql`now()`).notNull())
    .addColumn("updated_at", "timestamptz", (col) => col.defaultTo(sql`now()`).notNull())
    .addUniqueConstraint("games_active_chat_topic_id_unique", ["active_chat", "topic_id"])
    .execute();
  ...
  await db.schema
    ...
    .addColumn("word_length", sql`word_length`, (col) => col.notNull().defaultTo(sql`'5'::word_length`))
    .addColumn("created_at", "timestamptz", (col) => col.defaultTo(sql`now()`).notNull())
    .addColumn("updated_at", "timestamptz", (col) => col.defaultTo(sql`now()`).notNull())
    .execute();
  ...
  await db.schema
    ...
    .addColumn("id", "integer", (col) => col.primaryKey().generatedByDefaultAsIdentity())
    .addColumn("day_number", "integer", (col) => col.notNull().unique().generatedByDefaultAsIdentity())
    .addColumn("word", "varchar(6)", (col) => col.notNull())
  ...
  await sql`ALTER TABLE daily_words ADD CONSTRAINT daily_words_word_length_check CHECK (char_length(word) = 5);`.execute(db);

  await db.schema
    .createTable("daily_guesses")
    .addColumn("id", "integer", (col) => col.primaryKey().generatedByDefaultAsIdentity())
    .addColumn("user_id", "text", (col) => col.notNull().references("users.id").onDelete("cascade"))
    .addColumn("daily_word_id", "integer", (col) => col.notNull().references("daily_words.id").onDelete("cascade"))
    .addColumn("guess", "varchar(6)", (col) => col.notNull())
    .addColumn("attempt_number", "integer", (col) => col.notNull())
    .addColumn("created_at", "timestamptz", (col) => col.defaultTo(sql`now()`).notNull())
    .addColumn("updated_at", "timestamptz", (col) => col.defaultTo(sql`now()`).notNull())
    .addUniqueConstraint("daily_guesses_user_word_attempt_unique", ["user_id", "daily_word_id", "attempt_number"])
    .execute();

  // 6. Indexes
  await db.schema.createIndex("leaderboard_chat_word_len_idx").on("leaderboard").columns(["chat_id", "word_length", "created_at"]).execute();
  await db.schema.createIndex("leaderboard_user_word_len_idx").on("leaderboard").columns(["user_id", "word_length"]).execute();
  await db.schema.createIndex("guesses_game_id_idx").on("guesses").column("game_id").execute();
  await db.schema.createIndex("daily_guesses_user_daily_word_idx").on("daily_guesses").columns(["user_id", "daily_word_id"]).execute();

  // 7. Triggers
  const tables = ["users", "broadcast_chats", "chat_game_topics", "banned_users", "games", "guesses", "leaderboard", "user_stats", "daily_words", "daily_guesses"];
...
export async function down(db: Kysely<any>): Promise<void> {
  const tables = ["daily_guesses", "daily_words", "user_stats", "leaderboard", "guesses", "games", "banned_users", "authorized_users", "chat_game_topics", "broadcast_chats", "users"];
  for (const table of tables) {
